...
const BASE_URL = 'http://localhost:8080'; // Địa chỉ cơ sở của API

export async function getAllCategories(): Promise<CategoryModel[]> {
    try {
        const response: AxiosResponse<{ _embedded: { categories: CategoryModel[] } }> = await axios.get(`${BASE_URL}/category`);
...
export async function createCategory(newCategoryData: CategoryModel): Promise<CategoryModel> {
    try {
        const response: AxiosResponse<CategoryModel> = await axios.post<CategoryModel>(`${BASE_URL}/category`, newCategoryData);
...
export async function updateCategory(idCategory: number, updatedCategoryData: CategoryModel): Promise<CategoryModel> {
    try {
        const response: AxiosResponse<CategoryModel> = await axios.put<CategoryModel>(`${BASE_URL}/category/${idCategory}`, updatedCategoryData);
        return response.data;
    } catch (error) {
...
export async function deleteCategory(idCategory: number): Promise<void> {
    try {
        await axios.delete(`${BASE_URL}/category/${idCategory}`);
    } catch (error) {
